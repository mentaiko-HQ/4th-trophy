...
import { 
  Users, Settings, Megaphone, Save, Search, 
  UserX, RefreshCw, Trophy, Activity, AlertTriangle, 
  Database, Upload, Trash2, FileText, Download, RotateCcw, Archive, History, PlayCircle, Check, Target
} from 'lucide-react';

import { PlayerData, TournamentSettings } from '../../ranking/components/ScoreList'; // パスは実際の配置に合わせて調整してください
...
export default function AdminPanel({ initialPlayers, initialSettings }: AdminPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'players' | 'settings' | 'data'>('dashboard');
  ...
  const downloadCSV = (content: string, filename: string) => {
    const blob = new Blob([new Uint8Array([0xEF, 0xBB, 0xBF]), content], { type: 'text/csv;charset=utf-8;' }); // BOM付きUTF-8
  ...
  const handleExportData = () => {
    ...
    // ヘッダー
    const header = "No,氏名,所属,段位,所作,AM1立順,AM2立順,PM1立順,AM1的中,AM2的中,PM1的中,合計,暫定順位,最終順位,射詰,射遠順位,ステータス(AM1),欠席\n";
    ...
    downloadCSV(header + rows, `大会データ_${new Date().toISOString().slice(0, 10)}.csv`);
  };
  ...
  const handleExportScoreSheet = () => {
    ...
    downloadCSV(header + rows, `採点簿_AM1_${new Date().toISOString().slice(0, 10)}.csv`);
  };
  ...
  const handleResetScores = async () => {
    if (!window.confirm('全選手の「スコア」「順位」をクリアしますか？\n選手データや立順は保持されます。\n(リハーサル後の本番準備などに使用します)')) return;
  ...
  const handleResetStatuses = async () => {
    if (!window.confirm('全選手の進行状況(呼出/行射など)を「待機」に戻しますか？')) return;
  ...
  const handleCreateSnapshot = async () => {
    const label = prompt('バックアップの名前を入力してください (例: 予選終了後)');
    if (!label) return;

    setIsSaving(true);
    try {
      const { error } = await supabase.rpc('create_snapshot', { p_label: label });
      if (error) throw error;
      alert('バックアップを作成しました。');
      fetchSnapshots();
    } catch (error) {
  ...
  const handleRestoreSnapshot = async (id: string, label: string) => {
    if (!window.confirm(`【警告】現在のデータを破棄し、バックアップ「${label}」の状態に戻します。\n本当によろしいですか？`)) return;

    setIsSaving(true);
    try {
      const { error } = await supabase.rpc('restore_snapshot', { p_snapshot_id: id });
      if (error) throw error;
      alert('データを復元しました。');
      router.refresh();
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-3xl border-4 border-bk-brown shadow-[8px_8px_0px_0px_rgba(80,35,20,0.15)] flex flex-col h-full">
            <div className="flex items-center justify-between mb-6 border-b-2 border-bk-brown/10 pb-4">
                ...
                <label className="flex items-center cursor-pointer text-sm">
                    <span className="mr-2 text-bk-brown font-bold">{settings.show_phase ? '表示中' : '非表示'}</span>
                    <div className="relative">
                        ...
                        />
                        <div className={`block w-12 h-7 rounded-full transition-colors border-2 border-bk-brown ${settings.show_phase ? 'bg-bk-green' : 'bg-gray-300'}`}></div>
                        <div className={`dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full border-2 border-bk-brown transition-transform ${settings.show_phase ? 'transform translate-x-5' : ''}`}></div>
                    </div>
                </label>
            </div>
            ...
            <div className="space-y-3 mb-6 flex-grow">
              {[
                { val: 'preparing', label: '準備中。。。', color: 'bg-teal-100 border-teal-700 text-teal-900' },
                { val: 'qualifier', label: '予選進行中', color: 'bg-blue-100 border-blue-700 text-blue-900' },
                { val: 'tally', label: '集計中', color: 'bg-bk-orange border-bk-brown text-bk-brown' },
                { val: 'final', label: '決勝進行中', color: 'bg-bk-red border-bk-brown text-white' },
                { val: 'finished', label: '全日程終了', color: 'bg-gray-200 border-gray-600 text-gray-800' },
              ].map((phase) => (
                <label key={phase.val} className={`
                ...
                `}>
                  ...
                  `}>
                    {settings.current_phase === phase.val && <div className="w-3 h-3 bg-bk-brown rounded-full" />}
                  </div>
                  <span className="font-black text-lg">{phase.label}</span>
                  {settings.current_phase === phase.val && <span className="ml-auto text-xs bg-bk-brown text-white px-3 py-1 rounded-full font-bold">NOW</span>}
                </label>
              ))}
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border-4 border-bk-brown shadow-[8px_8px_0px_0px_rgba(80,35,20,0.15)] flex flex-col h-full">
            <div className="flex items-center justify-between mb-6 border-b-2 border-bk-brown/10 pb-4">
                ...
                <label className="flex items-center cursor-pointer text-sm">
                    <span className="mr-2 text-bk-brown font-bold">{settings.show_announcement ? '表示中' : '非表示'}</span>
                    <div className="relative">
                        <input 
                            type="checkbox" 
                            className="sr-only" 
                        ...
                        />
                        <div className={`block w-12 h-7 rounded-full transition-colors border-2 border-bk-brown ${settings.show_announcement ? 'bg-bk-orange' : 'bg-gray-300'}`}></div>
                        <div className={`dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full border-2 border-bk-brown transition-transform ${settings.show_announcement ? 'transform translate-x-5' : ''}`}></div>
                    </div>
                </label>
            </div>

            <div className="mb-4 flex-grow">
                <p className="text-xs font-bold text-gray-500 mb-2 uppercase">Message to Display</p>
                <textarea
      ...
      {activeTab === 'players' && (
        <div className="space-y-6">
          <div className="bg-yellow-50 border-4 border-bk-orange p-5 rounded-2xl shadow-sm flex items-start">
            <AlertTriangle className="text-bk-orange mr-4 shrink-0 mt-0.5" size={28} strokeWidth={2.5} />
            <div className="text-sm text-bk-brown">
              <p className="font-black text-lg mb-1">WARNING: 欠席処理</p>
              <p className="font-medium leading-relaxed">
                「欠席にする」ボタンを押すと、対象者はリストから除外され、<strong>後続の全選手の立順が自動的に繰り上がります。</strong>
                <br />
          ...
          <div className="bg-white rounded-3xl border-4 border-bk-brown shadow-[8px_8px_0px_0px_rgba(80,35,20,0.15)] overflow-hidden">
            <div className="p-5 border-b-4 border-bk-brown bg-bk-beige/20 flex flex-col sm:flex-row justify-between items-center gap-4">
              <h2 className="text-xl font-black text-bk-brown flex items-center uppercase">
                <Users className="mr-3" /> Player List <span className="ml-2 text-sm bg-white px-3 py-1 rounded-full border-2 border-bk-brown">{filteredPlayers.length}</span>
              </h2>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y-2 divide-gray-100">
                  {filteredPlayers.map((player: any) => (
                    <tr key={player.id} className={`transition-colors ${player.is_absent ? 'bg-gray-50' : 'hover:bg-bk-beige/10'}`}>
                      <td className="px-6 py-4 font-black text-bk-brown font-pop text-lg">{player.bib_number}</td>
                      <td className="px-6 py-4">
                        <div className="font-bold text-base text-bk-brown">{player.participants?.name}</div>
                        <div className="text-xs font-bold text-gray-500">{player.participants?.teams?.name}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        {!player.is_absent && (
                          ...
                          >
                            <UserX size={16} className="mr-2" strokeWidth={3} /> 欠席にする
                          </button>
                        )}
                        {player.is_absent && (
                  ...
                  {filteredPlayers.length === 0 && (
                      <tr>
                          <td colSpan={4} className="px-6 py-12 text-center text-gray-400 font-bold text-lg">
                              NO DATA
                          </td>
                      </tr>
                  )}
          ...
          <div className="grid grid-cols-1 gap-8 max-w-lg">
            <div>
              <label className="block text-base font-black text-bk-brown mb-3 flex items-center">
                <Trophy size={20} className="mr-2 text-bk-orange fill-bk-orange" /> 個人入賞枠数
              </label>
              <p className="text-xs font-bold text-gray-500 mb-3">上位何位までを入賞（表彰対象）とするか設定します。</p>
              <div className="flex items-center gap-3">
                <input
                  type="number"
                  min="1"
                  value={settings.individual_prize_count || 0}
                  onChange={(e) => handleSettingChange('individual_prize_count', Number(e.target.value))}
            ...
            <div className="border-t-2 border-bk-brown/10 pt-6">
              <label className="block text-base font-black text-bk-brown mb-3 flex items-center">
                <Target size={20} className="mr-2 text-bk-red" /> 順位決定戦（競射）対象者の表示
              </label>
              <p className="text-xs font-bold text-gray-500 mb-3">成績閲覧ページで競射対象者を自動抽出して表示するか設定します。</p>
              <label className="flex items-center cursor-pointer text-sm">
                  <span className="mr-3 text-bk-brown font-bold text-base">{settings.show_playoff_players ? '表示する' : '表示しない'}</span>
                  <div className="relative">
                      <input 
                          type="checkbox" 
                          className="sr-only" 
                          checked={settings.show_playoff_players ?? false}
                          onChange={(e) => handleSettingChange('show_playoff_players', e.target.checked)}
                      />
                      <div className={`block w-14 h-8 rounded-full transition-colors border-2 border-bk-brown ${settings.show_playoff_players ? 'bg-bk-red' : 'bg-gray-300'}`}></div>
                      <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full border-2 border-bk-brown transition-transform ${settings.show_playoff_players ? 'transform translate-x-6' : ''}`}></div>
                  </div>
              </label>
            </div>
          </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-3xl border-4 border-bk-brown shadow-[8px_8px_0px_0px_rgba(80,35,20,0.15)]">
            ...
            <div className="space-y-3">
                {snapshots.length === 0 && <p className="text-gray-400 font-bold text-center py-6">バックアップデータはありません。</p>}
                {snapshots.map(snap => (
                    <div key={snap.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl border-2 border-gray-200 hover:border-bk-brown/30 transition-colors">
                        <div>
                            <p className="font-bold text-bk-brown text-lg">{snap.label}</p>
                            <p className="text-xs font-bold text-gray-400 mt-1">{new Date(snap.created_at).toLocaleString()}</p>
                        </div>
                        <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-3xl border-4 border-bk-brown shadow-sm">
                ...
                <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4 mb-4">
                  <div className="flex items-start">
                    <FileText className="text-blue-600 mr-2 mt-1 shrink-0" size={20} />
                ...
                <div className="flex items-center justify-center w-full">
                  <label className="flex flex-col items-center justify-center w-full h-32 border-4 border-gray-300 border-dashed rounded-2xl cursor-pointer bg-gray-50 hover:bg-white hover:border-bk-brown transition-colors group">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      ...
                      <p className="mb-1 text-sm text-gray-500 font-bold group-hover:text-bk-brown">Click to upload CSV</p>
                    </div>
                    <input 
                        ref={fileInputRef}
                        type="file" 
              ...
              {/* エクスポート */}
              <div className="bg-white p-6 rounded-3xl border-4 border-bk-brown shadow-sm">
                <h2 className="text-lg font-black text-bk-brown mb-4 flex items-center uppercase"><Download className="mr-2 text-bk-green" /> Data Export</h2>
                <div className="space-y-3">
                    <button onClick={handleExportData} className="w-full flex items-center justify-center py-4 border-2 border-bk-brown rounded-xl hover:bg-bk-beige text-sm font-black text-bk-brown transition-all shadow-sm active:translate-y-0.5">
                        <FileText size={18} className="mr-2 text-gray-500"/> ALL DATA (Backup)
                    </button>
                    <button onClick={handleExportScoreSheet} className="w-full flex items-center justify-center py-4 border-2 border-bk-brown rounded-xl hover:bg-bk-beige text-sm font-black text-bk-brown transition-all shadow-sm active:translate-y-0.5">
                        <FileText size={18} className="mr-2 text-gray-500"/> SCORE SHEET (AM1)
                    </button>
                </div>
              </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border-4 border-bk-red shadow-sm">
            <h2 className="text-lg font-black text-bk-red mb-4 flex items-center uppercase"><AlertTriangle className="mr-2" /> DANGER ZONE</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <button onClick={handleResetScores} disabled={isSaving} className="py-3 px-4 rounded-xl bg-red-50 text-bk-red font-black text-sm hover:bg-red-100 flex items-center justify-center border-2 border-red-200">
                    <RotateCcw size={16} className="mr-2"/> スコアのみクリア
                </button>
                <button onClick={handleResetStatuses} disabled={isSaving} className="py-3 px-4 rounded-xl bg-red-50 text-bk-red font-black text-sm hover:bg-red-100 flex items-center justify-center border-2 border-red-200">
                    <RotateCcw size={16} className="mr-2"/> 進行状況リセット
                </button>
                <button onClick={handleResetData} disabled={isSaving} className="py-3 px-4 rounded-xl bg-bk-red text-white font-black text-sm hover:bg-[#a31b00] flex items-center justify-center shadow-md border-2 border-bk-brown">
